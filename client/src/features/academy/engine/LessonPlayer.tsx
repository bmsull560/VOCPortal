...
export function LessonPlayer({ lessonId }: LessonPlayerProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <header className="mb-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <span className="inline-flex items-center rounded-full border px-2 py-0.5">
              Roles: {lesson.targetRoles.join(", ")}
            </span>
            <span className="inline-flex items-center rounded-full border px-2 py-0.5">
              Level L{lesson.minMaturity}
            </span>
      ...
      {/* AI Quiz Generator - Available for all lessons */}
      <DynamicQuizGenerator
        lessonId={lesson.id}
        lessonTitle={lesson.title}
        userRole={lesson.targetRoles[0] === "All" ? "Sales" : lesson.targetRoles[0]}
        maturityLevel={lesson.minMaturity}
      />
    </div>
  );
}
